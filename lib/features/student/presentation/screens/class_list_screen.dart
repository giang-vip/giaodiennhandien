  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: viewModel.isLoading
          ...
          : classes.isEmpty
          ? const Center(child: Text("Không có lớp học nào để đăng ký.\nHoặc bạn đã đăng ký hết các lớp!", textAlign: TextAlign.center))
          : ListView.builder(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      bool success = await mainContext.read<ClassListViewModel>().registerClass(item.id);
                      if (success && mainContext.mounted) {
                        _showTopNotification(mainContext, 'Đăng ký lớp thành công!', Colors.green, Icons.check_circle);
                        await Future.delayed(const Duration(milliseconds: 1500));
                        if (mainContext.mounted) Navigator.pushReplacementNamed(mainContext, AppRoutes.registeredClasses);
                      } else if (mainContext.mounted) {
                        _showTopNotification(mainContext, 'Lỗi đăng ký lớp! Vui lòng thử lại.', Colors.red, Icons.error_outline);
                      }
                    },
                    ...
                    child: const Center(child: Text('ĐĂNG KÝ NGAY', style: TextStyle(fontWeight: FontWeight.bold))),
                  )
                ],
              ),
            ),
